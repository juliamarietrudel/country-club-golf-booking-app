...
export async function sendInvitation(email: string, name: string, link: string) {
  const result = await client().emails.send({ from: sender(), to: email, subject: "Vos journees de golf - Country Club de Montreal", html: `<p>Bonjour ${name},</p><p>Veuillez choisir les journees ou vous souhaitez jouer la semaine prochaine.</p><p><a href="${link}">Choisir mes journees</a></p><p>Les modifications sont possibles jusqu'a dimanche midi.</p><p>Vincent Trudel<br>Country Club de Montreal</p>` });
