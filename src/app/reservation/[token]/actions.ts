...
import { bookingIsOpen } from "@/lib/dates";
...
export async function saveBooking(formData: FormData) {
  ...
  if (!invitation || new Date(invitation.expires_at) < new Date() || !bookingIsOpen(invitation.week_start)) throw new Error("Cette reservation est fermee.");
  const allowed = await bookingDatesFor(invitation.week_start);
