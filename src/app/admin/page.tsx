...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ error?: string; sent?: string; resend?: string }> }) {
  ...
  return <main className={styles.page}>
    ...
    {sent !== undefined && <p className={styles.success}>{resend === "1" ? `${sent} invitation${sent === "1" ? " a ete renvoyee" : "s ont ete renvoyees"} aux golfeurs selectionnes.` : `${sent} invitation${sent === "1" ? " traitee" : "s traitees"}.`} Verifiez Resend pour confirmer la livraison.</p>}
    ...
    <section className={styles.card}><h2>Golfeurs</h2><p className={styles.statusHelp}><strong>Actif</strong> : recoit les invitations et les rappels. <strong>Desactive</strong> : reste dans l&apos;historique, mais ne recoit plus de courriel.</p><div className={styles.table}>{golfers.map((golfer) => <div className={`${styles.row} ${!golfer.active ? styles.inactiveRow : ""}`} key={golfer.id}><form action={editGolfer} className={styles.golferForm}><input type="hidden" name="id" value={golfer.id} /><input name="firstName" defaultValue={golfer.first_name} aria-label="Prenom" required /><input name="lastName" defaultValue={golfer.last_name} aria-label="Nom" required /><input name="email" type="email" defaultValue={golfer.email} aria-label="Courriel" required /><button className={styles.textButton}>Enregistrer</button></form><form action={toggleGolfer}><input type="hidden" name="id" value={golfer.id} /><input type="hidden" name="active" value={String(!golfer.active)} /><button className={styles.textButton}>{golfer.active ? "Desactiver" : "Reactiver"}</button></form></div>)}</div></section>
